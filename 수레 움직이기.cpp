...
bool isOutOfBounds(int y, int x) { return y < 0 || y >= N || x < 0 || x >= M; }
bool collapsed(int y, int x, int y2, int x2) { return y == y2 && x == x2; }
bool overlapped(pair<int, int> redPos, pair<int, int> nextRedPos,
                pair<int, int> bluePos, pair<int, int> nextBluePos) {
  return redPos == nextBluePos && bluePos == nextRedPos;
}
bool bothArrivedAtGoals(pair<int, int> redPos, pair<int, int> bluePos,
                        vector<vector<int>> &maze) {
  return maze[redPos.first][redPos.second] == RED_GOAL &&
         maze[bluePos.first][bluePos.second] == BLUE_GOAL;
}
void func(pair<int, int> redPos, pair<int, int> bluePos, int cnt,
          vector<vector<int>> &maze) {
  ...
  for (int k = 0; k < 5; k++) {
    int nextRedY = redPos.first + moveDir[k].y,
        nextRedX = redPos.second + moveDir[k].x;

    if (isOutOfBounds(nextRedY, nextRedX) ||
        (maze[nextRedY][nextRedX] != RED_GOAL &&
         visited[nextRedY][nextRedX][RED]) ||
    ...
    for (int l = 0; l < 5; l++) {
      ...
      if (isOutOfBounds(nextBlueY, nextBlueX) ||
          (maze[nextBlueY][nextBlueX] != BLUE_GOAL &&
           visited[nextBlueY][nextBlueX][BLUE]) ||
          maze[nextBlueY][nextBlueX] == WALL ||
          collapsed(nextRedY, nextRedX, nextBlueY, nextBlueX) ||
          overlapped(redPos, {nextRedY, nextRedX}, bluePos,
                     {nextBlueY, nextBlueX})) {
        continue;
      }
