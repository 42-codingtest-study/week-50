...
class Time {
 public:
  ...
  int toSeconds() { return h * 3600 + m * 60 + s; }
...
bool hourMatch(vector<double> cnt, vector<double> next) {
  return (cnt[0] > cnt[2] && next[0] <= next[2] ||
          cnt[2] == 354 && cnt[0] > 354);
}
bool minuteMatch(vector<double> cnt, vector<double> next) {
  return (cnt[1] > cnt[2] && next[1] <= next[2] ||
          cnt[2] == 354 && cnt[1] > 354);
}
int solution(int h1, int m1, int s1, int h2, int m2, int s2) {
  ...
  for (int i = start; i < end; i++) {
    ...
    answer = (hMatch && mMatch) ? (next[0] == next[1]) ? answer + 1 : answer + 2
             : (hMatch || mMatch) ? answer + 1
                                  : answer;
  }
